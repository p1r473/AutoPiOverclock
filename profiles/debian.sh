...
apo_profile_prove_network_watchdog_reboot() {
    ...
    if (( APO_NETWORK_WATCHDOG_REBOOT_COUNT == 1 )); then
        APO_NETWORK_WATCHDOG_PROOF_REASON="Project-owned Debian watchdog evidence proves one liveness-target reboot from boot $old_boot to boot $new_boot."
    else
        APO_NETWORK_WATCHDOG_PROOF_REASON="Project-owned Debian watchdog evidence proves $APO_NETWORK_WATCHDOG_REBOOT_COUNT consecutive liveness-target reboots from boot $old_boot to boot $new_boot."
    fi
}
