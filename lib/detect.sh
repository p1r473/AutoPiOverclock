...
apo_store_discovery_state() {
    ...
    for key in MODEL COMPATIBLE ARCH OS_ID OS_VERSION NORMAL_VOLTAGE_SOURCE TRYBOOT_EXISTS TRYBOOT_TYPE TRYBOOT_HASH BOOT_WATCHDOG_TIMEOUT KERNEL_WATCHDOG_TIMEOUT RUNTIME_WATCHDOG WATCHDOG_DEVICE WATCHDOG_RUNTIME_TIMEOUT WATCHDOG_OWNER ROOT_SOURCE BOOT_SOURCE DISPLAY_PRESENT AUDIO_BASELINE CPU_STRESS_AVAILABLE GPU_STRESS_AVAILABLE STRESS_NG_BINARY STRESS_NG_GPU_AVAILABLE DRM_RENDER_NODE OPENSSL_BINARY GLMARK_BINARY GLMARK_DATA RECENT_THROTTLED THROTTLE_RECENT_SUPPORTED; do
    ...
    case $APO_PROFILE in
        ...
        batocera)
            printf 'openssl=%s; glmark-binary=%s; glmark-data=%s' \
                ...
                "${APO_DISCOVERY[GLMARK_BINARY]:-missing}" \
