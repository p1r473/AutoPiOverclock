...
apo_classify_output() {
    ...
    if [[ -z $APO_LAST_CLASS ]]; then
        ...
        elif grep -Eqi 'under.?voltage|Current or new throttle/power flag|thermal thrott|I/O error|Buffer I/O error|EXT4-fs (error|warning)|device offline|reset (SuperSpeed|high-speed|full-speed) USB|v3d.*(hang|fault|timeout)|drm.*(hang|fault|timeout)|Oops:|BUG:|Call trace|segfault' "$output_file"; then
