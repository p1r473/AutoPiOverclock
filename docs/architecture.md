...
PREPARE → TRYBOOT_PROOF → GPU_SMOKE → CPU_SWEEP/refinement
        ...
        → COMBINED_VALIDATION(saved final duration)
        → optional COMBINED_CPU_EDGE(saved edge duration) → COMPLETE

PREPARE → TRYBOOT_PROOF → GPU_SMOKE → MANUAL_TEST(one exact CPU/GPU pair) → COMPLETE
```

Skipped explicit-plan domains are bypassed. Within each candidate, state records the candidate identity and a generic `BOOT_n`/`NORMAL_n` substage across the configured `candidate_boots` cycles, followed by the stress boot, stress, post-stress health, and final normal recovery. Automatic refinement indices, failure boundaries, guard verification, exact saved-duration CPU/GPU qualification identities, and monotonic backoff histories are resumable. Final validation checkpoints one combined CPU/GPU/I/O workload for the saved final duration, return to normal, the configured `final_boots` post-stress `BOOT_n`/`NORMAL_n` cycles, and final hash verification. Optional edge mode records the completed production floor before starting its saved edge-duration combined workload so a safely recovered edge failure cannot erase that result. The public defaults are 2 hours per qualification, 8 hours for final validation, and 24 hours when edge testing is requested.
