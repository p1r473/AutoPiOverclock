...
A maximum observed pass is never called “rock solid.” Configuration-free auto refines the failure gap at 25 MHz, then candidate-tests a production clock 50 MHz below the CPU boundary and 25 MHz below the GPU boundary. Explicit plans retain their configured list-position backoff. Final clocks are recorded only after CPU-only validation, GPU-only validation whenever GPU load is required, combined endurance, filesystem activity, post-stress health, the configured `final_boots` additional candidate boots and normal recoveries, and a final permanent-config hash check. The approved floor is three final boot/recovery cycles.

The 3200 MHz CPU and 1200 MHz GPU/V3D limits are sweep ceilings, not safe recommendations or promises that a target can boot them. Configuration-free auto refuses to generate candidates unless the active Pi 5 tuple is CPU 2400 MHz, V3D 800 or 960 MHz, and zero voltage delta, and a before/after-hashed audit of the permanent root config finds no explicit clock or voltage control. This covers boost/turbo/fixed-clock controls, every `*_freq`/`*_freq_min` assignment, and every `over_voltage*` assignment. Any snapshot change or `include` directive fails closed because unbound content cannot prove the protected baseline. Existing overclocks and unknown defaults fail preflight; permanent configuration is never rewritten automatically. Tryboot recovery and every normal stability boundary remain mandatory.

Optional `--edge-cpu-24h` runs only after the ordinary 50 MHz-buffered CPU production floor has completed its full eight-hour validation. It then raises only CPU by 25 MHz and repeats the complete validation with 24-hour endurance. A recovered `BOOT_FAILURE` or `STABILITY_FAILURE` rejects the experiment while preserving the validated floor. `HARNESS_FAILURE` or `RECOVERY_FAILURE` remains fatal because it does not establish an edge boundary safely.
...
Permanent apply requires current validation-schema evidence, `PASS`/`COMPLETE` state, and proof that at least 28,800 final-endurance seconds actually completed. An accepted optional edge result requires exactly 86,400 seconds; a rejected edge retains the separately recorded 28,800-second floor. Apply persists the old and proposed hashes, deterministic backup path, boot ID, and recovery intent before remote mutation. Resume reconciles a known old hash by fresh normal-boot health, a known proposed hash by fresh normal-boot health or verified rollback, and refuses to overwrite an unknown hash. Rollback passes the persisted proposed hash into the locked worker and verifies that exact destination hash both on entry and immediately before replacing it with the verified backup.
