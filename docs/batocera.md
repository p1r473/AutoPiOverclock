...
AutoPiOverclock requires a working hardware-watchdog recovery chain before tuning. `prepare` reads the active EEPROM timeout, kernel handoff, watchdog device, runtime timeout, and userspace owner. A missing hardware-watchdog condition stops before any candidate boot and must be configured separately from AutoPiOverclock. `prepare` never changes the existing hardware owner or its permanent configuration.

When a recognized permanent Batocera network/hardware keeper is active, discovery records its configured liveness target, service state, and installed hashes and uses it without changing it. If an unrelated process retains hardware ownership and no recognized network watcher exists, a confirmed tuning run installs a separate temporary network-only companion aimed at the current IPv4 default gateway. It never opens the hardware-watchdog device. If a registered project keeper is intentionally stopped and the device is unowned, the run instead installs a self companion that copies the permanent target and timing values, temporarily becomes the sole hardware owner and network decision maker, and records that mode in the run checkpoint. The installer verifies the permanent keeper, service, and config hashes without editing them, swaps service registration so only one project provider can start, and restores the unchanged permanent provider after successful cleanup. Interrupted apply and cleanup retain a proved provider for safe retry.

Strict network attribution requires a complete ordered chain of fresh durable events from the saved source boot to the current boot. Every edge binds its source and destination boot IDs, exact liveness target, unchanged installed hashes, event identity and time, active service, hardware-ownership mode, and matching prepared, committed, and next-boot-accepted records. A permanent keeper or self companion must additionally prove every committed hardware-watchdog starvation action. A network-only companion instead proves every accepted Batocera reboot request while another process remains the hardware owner. Missing, stale, late, changed, or contradictory evidence makes the reboot unattributed. A run-owned companion is retained through interruption or failure for safe resume and removed after successful completion; backups, logs, evidence, and permanent watchdog files remain.

When that proof is complete, only workload seconds explicitly reported by the target before the watcher request are retained. AutoPiOverclock restarts the same clocks for the exact remaining duration. SSH-outage time and controller estimates receive no credit. An unattributed reboot grants no time from its interrupted segment, retains only an earlier proof-bound checkpoint for the identical gate, and receives one same-clock replay of the uncredited remainder before a repeated identical event may enter ordinary stability isolation.
