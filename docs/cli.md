...
|---|---|---|
| `prepare TARGET` | Detect the supported Pi 5 platform and mode, install missing stress dependencies, install or repair watchdog recovery when required, and reboot for activation when needed. | Before a first run, after target software changes, or with `--dry-run` for a read-only readiness check. |
...
autopioverclock overclock pi@hostname --cpu-min 2900 --gpu-min 1050
...
| `--cpu-min MHZ` | history-guided or normal CPU start | Set the lowest new CPU candidate; 25 MHz aligned, above the protected clock, and no higher than `--cpu-max`. Rejected by `--gpu-only`. |
| `--cpu-max MHZ` | `3200` | Inclusive CPU candidate ceiling; 25 MHz aligned and no higher than 3200. Rejected by `--gpu-only`. |
| `--gpu-min MHZ` | history-guided or normal GPU start | Set the lowest new GPU/V3D candidate; 25 MHz aligned, above the protected clock, and no higher than `--gpu-max`. Rejected by `--cpu-only`. |
