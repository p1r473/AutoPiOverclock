...
| `overclock TARGET` | Rediscover the stock baseline, prove tryboot recovery, search CPU first, qualify CPU at stock GPU, search and qualify GPU, run combined CPU/GPU/I/O validation, adapt downward after eligible recovered boot/stability failures, retain/display the exact permanent diff, apply the validated result, reboot, and verify it. The recommended default durations are 2 hours per qualification and 8 hours for final validation. |
...
```bash
autopioverclock overclock TARGET --edge-hours 24
```

On a fresh run it validates the ordinary production floor first, then tests CPU exactly 25 MHz higher for the selected edge duration with combined CPU/GPU/I/O load and the production-floor GPU. If a normal `overclock TARGET` run has already completed and applied its current-schema floor, running the flagged command later creates a separate linked edge run, re-proves the live floor, and starts directly at edge validation. The completed final-validation phase is not repeated. The source run's automatically selected graphical or headless mode is retained exactly, so a target with no screen remains fully supported. A safely recovered edge boot/stability rejection keeps the validated floor; harness or recovery uncertainty remains fatal. `--edge-cpu-24h` remains a compatibility spelling for `--edge-hours 24`.
...
If its latest state is an eligible `overclock` interrupted after preflight, rerunning the same `autopioverclock overclock TARGET` command safely recovers and continues that run with its recorded cooling and duration policies. An interrupted workload is repeated at the same clocks because user interruption is not clock evidence. The same command also completes a validated application interrupted before or during reboot. During an unattended public overclock, exceeding the ordinary SSH timeout enters read-only polling instead of exiting or issuing another reboot; when SSH returns, the saved boot ID, tryboot ownership, normal clocks, and health are reconciled before tuning continues. A fully recovered `BOOT_FAILURE` or `STABILITY_FAILURE` during CPU/GPU qualification lowers only that domain. An ambiguous failure during combined production validation lowers every still-overclocked domain, repeats both qualifications, and restarts the complete saved final-validation duration without attributing cause. Duration-aware schema-10 state cannot be relabeled with different hours during continuation. Eligible schema-9 runs inherit their historical 2-hour qualification and 24-hour edge defaults while preserving their saved final duration; eligible schema-8 runs and recovered schema-7/8 final failures are migrated conservatively. Other failed, preflight-only, foreign, or ambiguous state is never silently adopted.
...
| `--final-hours HOURS` | `8` | Hours for combined final validation in `overclock`; whole numbers 1–168. |
| `--edge-hours HOURS` | off | Enable the final CPU +25 MHz combined CPU/GPU/I/O edge test for 1–168 hours. The recommended value is `24`. |
| `--edge-cpu-24h` | off | Compatibility spelling for `--edge-hours 24`; do not combine both forms. |
