...
The total estimate is dynamic rather than a promise: a newly discovered failure boundary can add 25 MHz refinement/guard tests or remove higher candidates, so the bar, ETA, and `tests left` count re-plan. Reboot/SSH work uses bounded historical estimates and is not shown as an exact countdown. Non-TTY/redirected execution prints ordinary raw telemetry, and candidate/main artifacts retain the raw worker lines in both modes.
