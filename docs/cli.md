...
| `--final-hours HOURS` | `48` for `overclock`; required duration alternative for `test` | Whole hours for automatic final validation or an exact-pair test; 1–168. |
...
| `--minutes MINUTES` | legacy duration alternative for `test` | Combined timed stress duration, 1–1440 minutes; cannot be combined with `--final-hours`. |
