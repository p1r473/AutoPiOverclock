...
|---|---|---|
| `prepare TARGET` | Detect the supported Pi 5 platform and mode, install missing stress dependencies, and verify the existing hardware-watchdog recovery chain. An existing recognized network watchdog is observed for reboot proof but never changed. | Before a first run, after target software changes, or with `--dry-run` for a read-only readiness check. |
...
If the target reboots during stress, AutoPiOverclock never assumes the clocks caused it and never assumes the network caused it. Network attribution is available only when a recognized watcher was already present at discovery and one fresh event binds the exact source and current boot IDs, configured liveness target, event identity and time, unchanged installed hashes, exact active service, and matching prepared, committed, and next-boot-accepted durable records. AutoPiOverclock reads that evidence but never installs or configures the watcher. Complete proof preserves only workload seconds explicitly reported by the target before the watchdog request, then runs the exact remainder at the same clocks without consuming the ordinary five-retry harness budget. Missing, late, or conflicting proof discards partial time and permits one conservative full same-clock replay. A second unattributed reboot of that identical gate can enter normal stability and CPU/GPU isolation only after protected normal recovery is fully proved.
