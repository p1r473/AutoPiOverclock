...
After `complete`, the sealed applied clocks are the protected floor. For clear failure boundary `F`, floor `P`, and resolution `R`, the highest automatic candidate is `P + floor((F - P - 1) / R) * R`, so the known failure is never retried by default. `F <= P` is contradictory and stops before mutation. A positive gap no larger than `R` has no legal resolution-aligned candidate, so that domain is skipped. The other domain may still tune; if neither domain has headroom, the run stops without changing the target. A failed pair forbids its northeast quadrant, so a floor at or beyond both failed coordinates is also contradictory.
