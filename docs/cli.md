...
After the original applied state has been deleted, `complete TARGET` without `--run-id` remains repeatable. With no later runs it reports that cleanup is already finished. It can also list and remove strictly validated later successful `prepare` audits, retained-history preflight failures, and successful no-headroom runs only when they own no stress job, watchdog helper, controller lease, or tryboot state. This repeat cleanup does not contact or change the target, permanent config, clocks, watchdogs, or durable ledger.
