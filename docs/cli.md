...
The command never removes or rewrites a permanent native Debian watchdog configuration, repair binary, or timeout. It also preserves the permanent Batocera watchdog. It removes only temporary companions, observers, wrappers, lease receipts, and run-owned harnesses. Unrelated boot settings, comments, and pre-existing fan settings are byte-preserved around the clock replacement. AutoPiOverclock's managed comments, completion hash marker, and temporary maximum-fan candidate settings are removed. The validated `over_voltage_delta`, `arm_freq`, and GPU key remain as ordinary settings under `[all]`.
