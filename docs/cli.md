...
On a real terminal, tuning and manual tests display one in-place controller-side progress line with the target, visual whole-job bar, approximate total percentage/ETA, approximate tests remaining, current clocks, current and run-maximum temperature, throttle state, and activity. During a timed workload it also displays the worker-reported countdown for that current stress phase. The bar erases and replaces only the current logical row, parks at column one, and emits no newline or vertical cursor movement, so phone-to-desktop Byobu/tmux resizing cannot strand an older display.
