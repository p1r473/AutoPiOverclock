...
autopioverclock complete TARGET
...
|---|---|---|
| `prepare TARGET` | Detect the supported Pi 5 platform and mode, install missing stress dependencies, and verify the existing hardware-watchdog recovery chain without changing watchdog configuration. | Before a first run, after target software changes, or with `--dry-run` for a read-only readiness check. |
...
| `complete TARGET` | Seal the validated applied result and failure history, simplify the permanent clock settings, and remove this target's disposable run data. | When a successful applied run no longer needs to remain resumable. |
...
Every `overclock TARGET` invocation is a new run. Before building its plan, AutoPiOverclock strictly validates the target's machine-ledger v2 failure file and every current-schema evidence-bearing automatic-run state for the exact target, profile, hardware identity, GPU key, baseline, and voltage policy. Unrelated audits and runs with no committed failure evidence are ignored. Older schemas and selection policies are not migrated or interpreted. Retained clear-domain failures supply hard exclusive domain ceilings unless the matching manual maximum is explicitly supplied. A failure at or below the sealed applied floor is contradictory and fails closed. When neither manual maximum is supplied for a full run, scalar ceilings are applied first. Only then may the newest nondominated ambiguous failed pair become the automatic reverse-search anchor. Explicit `--cpu-max` and `--gpu-max` values remain authoritative manual overrides after a warning. The failure ledger is created when missing and atomically replaced only if its derived content changed.
...
After `complete`, the sealed applied clocks are the protected floor. For active clear failure boundary `F`, floor `P`, and resolution `R`, the highest automatic candidate is `P + floor((F - P - 1) / R) * R`, so the known failure is never retried by default. A non-superseded `F <= P` is contradictory and stops before mutation. A positive gap no larger than `R` has no legal resolution-aligned candidate, so that domain is skipped. The other domain may still tune. If neither domain has a legal resolution-sized step outside every retained failed-pair frontier, the new run records a successful no-op without confirmation, reboot, clock change, stress, or apply. A failed pair forbids its northeast quadrant, so a non-superseded pair containing the floor is also contradictory.

After the required domain qualifications, the selected pair runs one saved-duration combined CPU/GPU/I/O validation, defaulting to 48 hours. CPU-specific evidence lowers only CPU by the CPU resolution; GPU-specific evidence lowers only GPU by the GPU resolution. If a full-mode clock failure is genuinely ambiguous after complete recovery, the failed pair becomes an anchor. AutoPiOverclock tries a CPU-only reduction first, then restores CPU and tries a GPU-only reduction, then tries both reductions while evidence remains ambiguous. Exact evidence immediately follows its identified domain. Each changed domain is requalified before another full final attempt, and each adjusted pair restarts the complete requested `--final-hours` duration from zero. Genuine clock, health, and workload failures receive no time credit. A strictly proved network-watchdog reboot at unchanged clocks instead retains only conservative target-reported completed work and runs the exact remaining duration. A hard minimum is never crossed; when the next required search, qualification, or isolation point would be below it, the run fails and reports the limiting domain and floor.
...
Both clocks and `--final-hours` are required. CPU accepts 600-4000 MHz, GPU/V3D accepts 200-3000 MHz, and `--final-hours` accepts 1-596523 whole hours. Both requested clocks must be at least the protected normal clocks and at least one must be higher; underclock/undervolt testing is outside this command. The requested duration is the combined timed stress duration, while recovery proof, a GPU harness smoke check, repeated candidate/normal boot cycles, post-stress health, and final normal recovery add wall time. The timed workload runs as a detached fixed-deadline job on the target and retains its result across controller SSH loss. Run `prepare TARGET` first if dependencies or watchdog recovery are not ready.

The command is intentionally not a shortcut to permanent configuration. It uses the existing voltage delta, sets `VALIDATED=0` and `APPLY_STATUS=NOT_APPLIED`, never creates a recommendation, and `apply` explicitly refuses its run ID. A timed pass means that exact workload and safety sequence passed for the requested duration; it does not satisfy an automatic run's qualification or final-validation plan. Rerunning the same clocks, duration, and cooling policy safely continues a current-schema interruption. Different clocks or duration cannot silently replace an interrupted manual plan; recover or finish the retained run first. `--no-max-fan` is accepted only when that cooling condition is intentional and cannot be changed during continuation.

## Interactive progress

On a real terminal, tuning and manual tests display one in-place controller-side progress line with the target, visual whole-job bar, approximate total percentage/ETA, approximate tests remaining, current clocks, current and run-maximum temperature, throttle state, and activity. During a timed workload its countdown changes every second. Target heartbeats supply fresh time and telemetry while connected. If SSH is lost, the same row becomes a rotating reconnect indicator with a seconds counter; it returns to the normal countdown after ownership-checked reattachment. Before repainting or writing ordinary output, the renderer recalculates how many physical rows the previous payload occupies at the current width and clears every reflowed fragment. It uses bounded downward movement only while clearing those fragments, never upward movement or saved-cursor restoration, then paints one clipped newline-free row and parks at column one. Inside tmux, it also sizes the line to the narrowest attached client. Terminal clearing and repainting are best effort: a rendering failure cannot change the controller's result, hide the next diagnostic, or terminate the job.

The total estimate is dynamic rather than a promise: a newly discovered failure boundary can add per-domain refinement tests or shorten a coarse sweep, so the bar, ETA, and `tests left` count re-plan. Reboot work uses bounded historical estimates. An SSH outage shows elapsed reconnect time, not invented target telemetry. `Ctrl-C` clears and suppresses the display before deliberate normal recovery begins. A controller `TERM`, `HUP`, crash, or reboot instead leaves exact detached-job ownership saved so `resume TARGET` can reattach without stopping the target workload. Non-TTY/redirected execution prints ordinary raw telemetry, and candidate/main artifacts retain the raw worker lines in both modes.

`prepare` and `overclock` are explicit authorization for the operations named by those commands. `prepare` may install missing workload dependencies but does not change watchdog configuration. After tuning confirmation, `overclock` may install only the network-proof component required for that run. A supported native Debian network watchdog receives a passive observer without changes to its configuration, repair binary, or timeouts. If no recognized target network watcher exists while another process owns the hardware device, an isolated temporary network-only gateway watcher is installed. On Batocera, a registered project keeper that is stopped may instead be replaced temporarily by a hash-bound self companion as the sole hardware and network provider; its permanent files and timing configuration remain unchanged and are restored after successful cleanup. Each controller run also leases either the controller's unchanged supported native Debian network watchdog or one shared temporary gateway companion. Unsupported active native network-watchdog configuration fails closed instead of receiving a second watcher. `overclock` may apply only the final result after current-schema validation and displays and retains the exact diff before applying it. Neither command overwrites unknown tryboot evidence or bypasses protected-hash checks.
...
If the target reboots during stress, AutoPiOverclock never assumes the clocks caused it and never assumes the network caused it. Network attribution is available only when the run's saved proof provider supplies a complete ordered event chain from the exact source boot to the current boot. Every edge binds its source and destination boot IDs, configured liveness target, event identity and time, unchanged installed hashes, exact active service, and matching prepared, committed, and next-boot-accepted durable records. Complete proof preserves only workload seconds explicitly reported by the target before the final proved watchdog request, then runs the exact remainder at the same clocks without consuming the ordinary five-retry harness budget. Missing, late, or conflicting proof grants no time from the interrupted segment, retains only valid earlier proof-bound credit for the identical gate, and permits one conservative same-clock replay of the uncredited remainder. A second unattributed reboot of that identical gate can enter normal stability and CPU/GPU isolation only after protected normal recovery is fully proved. Run-owned observers and watchers survive interruption or failure for resume and are removed after successful completion without removing native watchdogs, backups, logs, or evidence.
...
`resume TARGET` selects the latest retained state when `--run-id` is omitted. It also supports an explicit, bounded checkpoint restart for an active current-schema automatic overclock:
...
| `--no-history` | off | Create a fresh plan without scanning retained failures or creating or refreshing `history/failures.txt`. It deletes nothing. |
...
| `--output-dir DIR` | per-target XDG state directory | Use `DIR` as this target's custom state directory. Run artifacts are stored directly in `DIR`; durable history is stored in `DIR/history`. |
...
autopioverclock complete TARGET [--run-id RUN_ID]
autopioverclock reset TARGET
```
...
| `--run-id RUN_ID` | `resume`, `status`, `summary`, `recover`, `restore`, `apply`, `complete`, `report` | Select an older retained operation instead of the command's normal latest choice. |
...
`complete TARGET` is the only command that deletes retained run artifacts. It requires a current-schema fully validated applied tuning result, no live target stress ownership, exact permanent-config hash continuity, clear tryboot state, and typed confirmation in the form `COMPLETE TARGET_SLUG RUN_ID`. The command owns the exclusive target lock while classifying retained state, so it lists stale controller-only `RUNNING` or `PREPARING` checkpoints as abandoned instead of treating their saved text as a live controller. Retained target-side stress ownership remains a hard refusal. Before deletion it atomically seals the consolidated failure ledger and applied clock/config binding. It displays the exact config diff, simplifies the applied clock block, verifies the resulting hash and live target health, cleans run-owned watchdog helpers and harness files, then deletes controller logs, states, reports, evidence, and target backups for all validated retained runs belonging to that exact target.

After the original applied state has been deleted, `complete TARGET` without `--run-id` remains repeatable. It reconnects to the target and requires an exact match to the sealed hardware identity, boot paths, clock tuple, voltage, current schemas, watchdog readiness, normal boot, and clear throttle state. A structurally safe completed config with redundant global headers, or a manually canonicalized config with the same sealed tuple, is rendered into the canonical form and displayed as an exact diff. After the same typed confirmation, the command uses exact old and proposed hashes, a verified deterministic maintenance backup, immediate rollback on an install failure, post-install byte comparison, and an atomic durable-ledger hash rebind. It can also list and remove strictly validated later successful `prepare` audits, retained-history preflight failures, and successful no-headroom runs only when they own no stress job, watchdog helper, controller lease, or tryboot state.

The command never removes or rewrites a permanent native Debian watchdog configuration, repair binary, or timeout. It also preserves the permanent Batocera watchdog. It removes only temporary companions, observers, wrappers, lease receipts, and run-owned harnesses. Unrelated boot directives, comments, and pre-existing fan settings are preserved. AutoPiOverclock's managed comments, completion hash marker, temporary maximum-fan candidate settings, redundant global section headers, and empty section labels are removed. A global-only file receives exactly one `[all]` before its first active directive. Meaningful conditional sections and the `[all]` resets needed to leave them are preserved. The validated `over_voltage_delta`, `arm_freq`, and GPU key remain ordinary global settings.

After success, the deleted runs cannot be resumed or reported. The permanent `${XDG_STATE_HOME:-$HOME/.local/state}/autopioverclock/targets/TARGET_SLUG/history/failures.txt` remains. A later `prepare TARGET` validates the live completed floor against that ledger; a later `overclock TARGET` starts a new immutable plan above that floor while honoring its retained hard failure ceilings. A `resume` never rescans the ledger and continues only the plan saved in its selected run.

`status TARGET` reports the target's live active and measured CPU/GPU clocks, quick throttle evidence, temperature, uptime, tryboot state, controller-lock state, and a plain verdict such as `IN PROGRESS`, `STOCK / RESET`, `OVERCLOCKED / VALIDATED`, `OVERCLOCKED / UNVERIFIED`, or `RECOVERY NEEDED`, followed by the selected saved operation when one exists. Without `--run-id`, it uses the latest operation. After `complete` removes every saved operation, `status` still reports `OVERCLOCKED / VALIDATED` only when the strict sealed ledger's target, current schemas, hardware identity, boot paths, GPU key, voltage, clocks, and permanent-config hash exactly match the live snapshot.
