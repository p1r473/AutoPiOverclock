...
- Transient controller evidence is never a one-shot decision: scalar SSH facts, exact files, boot/tryboot identity, discovery, and the protected permanent hash receive 30 validated attempts separated by 10 seconds. Safe same-boot worker checks receive up to five attempts. If a complete candidate gate still loses harness evidence, the controller first proves full stock recovery and may repeat that exact gate five times; an exactly observed autonomous candidate reboot instead becomes a boot/stability boundary and backs off automatically. Every mutating/recovery command keeps polling read-only every 10 seconds after its ordinary 240/300-second boot timeout until SSH returns or the operator interrupts it. A valid hash mismatch, unknown tryboot ownership, or uncertain recovery is never retried away.
...
| Debian-family Raspberry Pi 5 run | Earlier-version tuning evidence exists, but a complete `alpha.39` default-sequence run remains pending. |
| Batocera Raspberry Pi 5 run | Recovery and watchdog preparation have been exercised, but complete `alpha.39` validation remains pending. |
