...
- `prepare` installs and verifies target prerequisites and recovery safeguards, including the project-owned network-reboot evidence components used on Debian-family and Batocera targets. It may update watchdog files and reboot the target.
- `overclock` starts a new search, uses compatible past failures to choose safe search ceilings, validates the result, applies it, reboots, and verifies it.

If a previous controller run was interrupted, use `autopioverclock resume pi@hostname` instead. In plain terms: use `overclock` when you want a new tuning decision; use `resume` when you want the same saved job to pick up where it stopped. If the controller reboots or loses its terminal during stress, run `resume` after it returns; the target-side deadline and retained result continue independently. `Ctrl-C` is different: it deliberately stops the active controller run and returns the target to normal clocks.
...
As of 2026-09-11, `prepare` and `overclock` are the two everyday commands, with `reset` available when a user wants to return to stock and start over. A fresh public overclock uses compatible retained failure evidence by default, searches and qualifies CPU first, then GPU, and runs one 48-hour combined final validation. A domain with no known clear ceiling uses the normal forward sweep. A clear retained ceiling or explicit maximum instead starts a complete reverse sweep at that ceiling. CPU coarse movement is normally 100 MHz and GPU coarse movement is normally 50 MHz; if a chosen resolution is larger, it also becomes that domain's coarse step. Ambiguous failed pairs remain pair constraints and use ordered CPU-only, GPU-only, then paired isolation rather than pretending either domain failed alone. Timed stress survives controller transport loss under a target-enforced deadline. Strictly proven project network-watchdog reboots replay the same clocks, while unproved reboots are never excused as network events. Raspberry Pi OS/Debian headless operation is automatic and does not require display or audio hardware.

| Evidence | Current status |
| --- | --- |
| Bash fixture suite | 24 scripted suites cover the normal/manual-test interface, retained-history planning, isolation scheduling, durable target stress jobs, live observers, progress calculations/rendering, installed entry point, state, classification, workers, tryboot, watchdog installation and reboot proof, selection, resume, apply, restore, reset, packaging, and public-safety contracts. |
...
| Batocera Raspberry Pi 5 run | Recovery and watchdog preparation have been exercised, but complete current-version end-to-end validation remains pending. |
...
Every `autopioverclock overclock TARGET` invocation creates a fresh run. It never silently resumes or changes an older run. Before building the new plan, it screens retained state metadata, ignores and preserves older or missing schemas, strictly validates compatible current-schema evidence-bearing automatic runs, and reuses proved CPU, GPU, and failed-pair boundaries. Unrelated audits and abandoned runs with no committed failure evidence are ignored. The planning notice shows the retained ceilings, requested maxima, selected search direction, retained evidence, and `target-failures.txt` path. Retained ceilings are safe defaults, not vetoes: an explicit `--cpu-max` or `--gpu-max` is authoritative for that domain. If it exceeds the retained domain ceiling, AutoPiOverclock warns clearly and still tests the requested ceiling. A fresh `--no-history` run skips the state scan and ledger update without deleting prior artifacts.
...
2. **Search CPU first, by itself.** With no clear retained or explicit CPU ceiling, the normal forward sweep starts at 2500 MHz and rises in 100 MHz coarse steps. When a clear CPU history boundary or `--cpu-max` supplies a ceiling, the reverse sweep tests that exact ceiling first and descends until it finds a pass. It then refines the proved pass/fail gap at `--cpu-resolution` granularity, 25 MHz by default, to find the highest actual pass. The coarse step is 100 MHz unless the selected resolution is larger. GPU remains at the protected baseline throughout this CPU search.
...
| `--cpu-max MHZ` / `--gpu-max MHZ` | `overclock` | Set an inclusive, authoritative ceiling and start that domain in reverse, warning rather than silently lowering it when history conflicts. Built-in maxima are CPU 3200 and GPU 1200 MHz. |
...
```bash
# Start a new normal run. Domains without ceilings sweep forward; retained ceilings sweep in reverse.
autopioverclock overclock pi@hostname

# Start at known ceilings, tune downward if needed, and require a fresh 100-hour final pass.
autopioverclock overclock pi@hostname --cpu-max 3075 --gpu-max 1200 --final-hours 100
...
Each candidate and final-validation substage is checkpointed atomically. The random tryboot ownership token, completed-file hash, reservation hash, token-specific quarantine path, and cooling policy are saved before remote creation can begin. Timed stress additionally saves a distinct job ID, 256-bit token, specification hash, source boot ID, phase, duration, and target start time before launch. An orderly controller exit still attempts normal recovery. An abrupt controller loss leaves the fixed-deadline target job running; `resume` reattaches to it instead of rebooting or starting a duplicate. Unknown or changed ownership evidence is preserved and fails closed.

An SSH transport timeout is not by itself a clock boundary. During timed stress, the target job continues while the controller polls indefinitely and shows a periodic notice; a same-boot return reattaches by exact ownership and collects the retained result. During candidate boot or required post-stress health, individual validated reads still receive 30 attempts with 10-second spacing and safe read-only/idempotent worker gates receive up to five same-boot attempts. The controller records the exact candidate boot before transient worker redeployment. A changed boot during stress is excused as network-caused only when the selected profile strictly proves a fresh project event bound to the exact old and new boot IDs, configured liveness target, installed asset hashes, timestamp, active service, and matching prepared, committed, and next-boot-accepted durable records. The keeper withdraws or invalidates pending intent if the requested reset does not begin inside its bounded window, preventing a later unrelated reboot from inheriting stale attribution. Valid proof is retried read-only while the service finishes startup, then repeats the complete gate at the same clocks without consuming the normal harness retry budget. Missing or contradictory proof permits one conservative full same-clock replay; a second unattributed reboot of the identical gate is handled as stability evidence only after complete normal recovery. Protected-hash, tryboot ownership, or recovery uncertainty still stops.

All live mutating/recovery commands allow a full five-minute SSH/reboot budget, then continue read-only polling every 10 seconds without treating the timeout as a clock result; a status notice is emitted every five minutes. During active stress the progress countdown keeps moving in seconds from the target start time, even while no telemetry is available. The target stops stress at its hard deadline even if SSH has not returned, retains completion evidence, and does not keep stressing indefinitely. A controller reboot or lost terminal leaves the exact saved target job resumable; `Ctrl-C` deliberately stops and recovers it. After every expected or observed target reboot, the controller re-uploads the run-isolated worker and helper before collecting health evidence, so volatile `/tmp` cleanup is handled automatically.
