...
- Did the guarded CPU pass two hours at stock GPU, did the guarded GPU pass two hours at that qualified CPU, and did the pair survive eight hours of combined CPU/GPU/I/O validation?
- When requested, did CPU 25 MHz higher survive a fresh 24-hour combined CPU/GPU/I/O edge validation at the production-floor GPU?
...
| Optional CPU edge | Production-floor CPU plus 25 MHz, tested for 24 hours under combined CPU/GPU/I/O load; a safely rejected edge retains the validated floor, while a pass becomes the final CPU result. |
...
CPU candidates climb from 2500 through 3200 MHz in 100 MHz steps using 10-minute candidate stress, and a real CPU boot/stability boundary is refined in 25 MHz steps. The candidate-tested CPU guard 50 MHz below the boundary or ceiling must then pass a two-hour CPU-only qualification with V3D held at its stock clock. Only after that succeeds does V3D climb in 50 MHz steps through 1200 MHz at the qualified CPU, refine a real boundary in 25 MHz steps, and candidate-test its 25 MHz guard. That GPU guard must pass a two-hour GPU-only qualification at the qualified CPU. Finally, the exact pair must pass eight hours of combined CPU/GPU/I/O load. The command retains and displays the permanent-config diff, applies only that fully validated result, reboots, and proves it.
...
On a fresh run, that first validates the ordinary guarded floor, then tests CPU exactly 25 MHz higher for 24 hours with the qualified production-floor GPU and combined CPU/GPU/I/O load. If the ordinary eight-hour result was already completed and applied without the flag, run the same command later: AutoPiOverclock links a new edge record to that retained floor, re-proves its live hash/health/tryboot state, and begins the 24-hour edge validation directly. It does not repeat the eight-hour endurance phase. The later edge run retains the source run's graphical or headless mode and uses a separate apply/rollback backup. It uses maximum fan cooling unless that new edge invocation explicitly includes `--no-max-fan`. A safely recovered edge boot/stability failure keeps the already-applied floor; harness or recovery uncertainty still stops the run.

The controller preserves the internal recovery proof, token/hash-owned `tryboot.txt` lifecycle, candidate/normal boot cycles, GPU harness, telemetry, artifacts, and resumable state. After the short CPU search, it proves the selected CPU for two hours with the GPU at stock before beginning the GPU search. It then proves the selected GPU for two hours at the qualified CPU before the eight-hour combined validation. A recovered CPU-qualification failure lowers CPU by 50 MHz; a recovered GPU-qualification failure lowers GPU by 25 MHz. A recovered combined-validation failure cannot identify one domain, so it lowers every still-overclocked domain by those guards and repeats both qualifications before restarting combined validation. Harness or recovery uncertainty still stops because it is not evidence of an unstable clock. Rerunning `autopioverclock overclock pi-host` continues its own latest safely resumable run and completes application. Ordinary users do not have to assemble a chain of `run`, dependency, watchdog, `resume`, and `apply` commands.
