...
As of 2026-08-28, `alpha.25` keeps `prepare`, `overclock`, and `reset` as the complete normal interface while retaining the fail-closed recovery engine underneath. It adds an advisory live whole-workflow display and a bounded exact-clock `test` command without turning short/manual evidence into a recommendation. It restores the transient worker automatically after every reboot, requests temporary maximum fan cooling by default without changing the permanent fan curve, preserves an already-positive Batocera EEPROM boot-watchdog timeout, and automatically binds Debian graphical validation to working audio evidence. A recovered final CPU-only or GPU-only boundary lowers that domain; a recovered ordinary combined-endurance boundary conservatively lowers both still-overclocked domains without pretending to know which caused the failure. Debian-family and Batocera runs still require completed current-schema artifact review, so this README intentionally makes no hardware-pass or production-clock claim from the UX release.
...
| Debian-family Raspberry Pi 5 run | Autonomous final-stress recovery has been observed, but a complete `alpha.25` run remains pending. |
| Batocera Raspberry Pi 5 run | Recovery-mode boot and watchdog preparation have been exercised, but complete `alpha.25` validation remains pending. |
...
The controller preserves the internal recovery proof, token/hash-owned `tryboot.txt` lifecycle, candidate/normal boot cycles, GPU harness, telemetry, artifacts, and resumable state. If final CPU-only or GPU-only stress proves a real stability boundary and normal recovery succeeds, it lowers that one clock by 50 MHz or 25 MHz. If ordinary combined endurance proves only that the pair failed, it lowers every still-overclocked domain by those same guards without blaming one domain. Either case restarts complete final validation. Boot, harness, or recovery uncertainty still stops. Rerunning `autopioverclock overclock pi-host` continues its own latest safely resumable run and completes application. Ordinary users do not have to assemble a chain of `run`, dependency, watchdog, `resume`, and `apply` commands.
