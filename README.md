...
- `overclock` starts a new search, uses compatible past failures to choose safe search ceilings, validates the result, applies it, reboots, and verifies it. After confirmation, it adds only the proof component needed for network-loss attribution: a passive observer for a supported native Debian network watchdog, or a temporary network-only gateway watcher when no recognized network watcher exists. It never replaces or reconfigures the hardware watchdog or an existing native watchdog.
...
> [!IMPORTANT]
> Batocera may require `prepare` to build its graphical payload on an ARM64 Debian-family controller. Its hardware-watchdog recovery chain must already be configured independently. AutoPiOverclock preserves a recognized existing network watchdog. If none exists, a confirmed tuning run installs a temporary network-only gateway companion that never opens the hardware-watchdog device and removes it after successful completion. Review [the Batocera notes](docs/batocera.md).
