...
- `prepare` installs missing workload dependencies and verifies the target's existing hardware-watchdog recovery chain. It never installs, enables, or configures a hardware or network watchdog.
...
> [!IMPORTANT]
> Batocera may require `prepare` to build its graphical payload on an ARM64 Debian-family controller. Its hardware-watchdog recovery chain must already be configured independently. If a recognized network watchdog already exists, AutoPiOverclock records it for read-only reboot attribution but never changes it. Review [the Batocera notes](docs/batocera.md).
...
| --- | --- | --- |
| `prepare TARGET` | Installs missing workload dependencies and verifies the existing hardware-watchdog recovery chain. It observes but never changes an existing network watchdog. | Setting up a target, checking prerequisites, or checking readiness with `--dry-run`. |
...
| `test TARGET --cpu MHZ --gpu MHZ --final-hours HOURS` | Tests one exact pair using `--final-hours` or legacy `--minutes`, then recovers normally; it never tunes or applies. | You already know the exact clocks and only want pass/fail evidence. |
...
Alpha.69 records a bounded timestamped history of distinct target workload samples while detached stress runs. If a recognized network watchdog is later proved to have requested the reboot, continuation credits the newest sample first observed at or before that request. Heartbeats or telemetry received during the watchdog's reset delay cannot erase that safe sample or add post-request time. Malformed, inconsistent, or late-only history receives no credit.
...
An SSH transport timeout is not by itself a clock boundary. During timed stress, the target job continues while the controller retries indefinitely on one in-place reconnect spinner; a same-boot return reattaches by exact ownership and collects the retained result. During candidate boot or required post-stress health, individual validated reads still receive 30 attempts with 10-second spacing and safe read-only/idempotent worker gates receive up to five same-boot attempts. The controller records the exact candidate boot before transient worker redeployment. A changed boot during stress is excused as network-caused only when a recognized watcher observed during discovery supplies a fresh event bound to the exact old and new boot IDs, configured liveness target, unchanged installed hashes, timestamp, active service, and matching prepared, committed, and next-boot-accepted durable records. AutoPiOverclock reads this evidence but never installs or configures the watcher. Valid proof is retried read-only while the existing service finishes startup. Only target-reported completed workload time observed before the watcher request is retained, and the unchanged pair runs the exact remaining duration. Wall-clock time and the SSH outage receive no credit. Missing, stale, late, or contradictory proof discards partial time and permits one conservative full same-clock replay; a second unattributed reboot of the identical gate is handled as stability evidence only after complete normal recovery. Protected-hash, tryboot ownership, or recovery uncertainty still stops.
