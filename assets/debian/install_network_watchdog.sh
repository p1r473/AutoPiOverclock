...
cmd_cleanup() {
    ...
    local current_config_hash current_keeper_hash current_service_hash expected_marker actual_marker live_owned=0
    ...
    current_keeper_hash=$(path_hash "$LIVE_KEEPER")
    current_service_hash=$(path_hash "$LIVE_SERVICE")
    if [[ $current_config_hash == "$config_hash" && $current_keeper_hash == "$keeper_hash" &&
          $current_service_hash == "$service_hash" ]] && grep -Fqx "RUN_ID=$run_id" "$LIVE_CONFIG"; then
        live_owned=1
    fi
    ...
    if [[ $actual_marker != "$expected_marker" ]]; then
        (( live_owned == 1 )) &&
            archive_stale_cleanup_marker "$cleanup_marker" "$run_id" KEEPER_SHA256 &&
            write_cleanup_marker "$cleanup_marker" "$expected_marker" || {
